...
class FirestoreService {
  ...
  static Future<void> completeDoctorStartFlow({
    ...
    try {
      ...
      debugPrint(
        '✅ STEP 1 COMPLETE: Doctor started consultation → Patient notified',
      );
    } catch (e) {
  ...
  static Future<void> completeDoctorJoinFlow({
    ...
    try {
      ...
      debugPrint(
        '✅ STEP 3 COMPLETE: Doctor joined → Both in consultation room',
      );
    } catch (e) {
  ...
  static Stream<List<Map<String, dynamic>>> getPatientSessionsStream(
    String patientId,
  ) {
    return _firestore
  ...
  static Stream<List<Map<String, dynamic>>> getDoctorSessionsStream(
    String doctorId, {
    String? scheduleId,
  }) {
    debugPrint(
      '📡 getDoctorSessionsStream called → doctorId=$doctorId, '
      'scheduleId=${scheduleId ?? 'NONE'}',
    );

    if (scheduleId == null || scheduleId.isEmpty) {
      debugPrint(
        'ℹ️ No scheduleId. Fetching all sessions for doctor $doctorId',
      );
    ...
    return _firestore
        .collection('sessions') // The collection containing the schedule doc
        .doc(scheduleId)
        .snapshots()
        .asyncExpand((scheduleSnapshot) {
          if (!scheduleSnapshot.exists) {
            debugPrint('⚠️ Schedule document does not exist: $scheduleId');
            return Stream.value([]);
          }

          // Extract appointmentIds
          final data = scheduleSnapshot.data() as Map<String, dynamic>?;
          final List<dynamic> rawIds = data?['appointmentIds'] ?? [];

          final List<String> appointmentIds = rawIds
              .map((e) => e.toString())
              .where((e) => e.isNotEmpty)
              .toList();

          // If the schedule has no appointments yet, return empty list
          if (appointmentIds.isEmpty) {
            debugPrint('ℹ️ Schedule $scheduleId has no appointments.');
            return Stream.value([]);
          }

          // Handle Firestore 'whereIn' limit (max 30)
          final idsToQuery = appointmentIds.take(30).toList();

          return _firestore
              .collection('telemedicine_sessions')
              .where('appointmentId', whereIn: idsToQuery)
              // Note: You usually don't need 'doctorId' here if appointmentId is unique,
              // but you can add .where('doctorId', isEqualTo: doctorId) for extra safety.
              .snapshots()
              .map((snapshot) => _processSnapshot(snapshot));
        });
  }

  // Helper method to process the snapshot and sort (avoids code duplication)
  static List<Map<String, dynamic>> _processSnapshot(QuerySnapshot snapshot) {
    debugPrint('📡 Processing ${snapshot.docs.length} telemedicine sessions');
  ...
  static Map<String, dynamic> _parseSessionData(
    String docId,
  ...
  static DateTime? _tryAlternativeDateFormats(String dateString) {
    try {
      if (dateString.toLowerCase().contains('today') ||
          dateString.toLowerCase().contains('tomorrow')) {
        final match = RegExp(
          r'(\d{1,2})/(\d{1,2})/(\d{4})',
  ...
  static Future<void> createDoctorStartedNotification({
    ...
    try {
      debugPrint(
        '🔔 Creating doctor started notification for patient: $patientId',
      );
      ...
      await _firestore.collection('notifications').add({
        ...
        'message':
            'Dr. $doctorName has started your $consultationType consultation',
  ...
  static Future<Map<String, dynamic>?> getSessionByAppointmentId(
  ...
  static Stream<List<Map<String, dynamic>>> getDoctorNotificationsStream(
    String doctorId,
  ) {
    return _firestore
  ...
  static Stream<List<Map<String, dynamic>>> getPatientNotificationsStream(
    String patientId,
  ) {
    return _firestore
        .collection('notifications')
  ...
  static Future<void> updateSessionJoinStatus({
    ...
    try {
      ...
      debugPrint(
        '✅ $userType join status updated: $hasJoined for $appointmentId',
      );
    } catch (e) {
  ...
  static Future<Map<String, dynamic>?> getSessionJoinStatus(
    String appointmentId,
  ) async {
    try {
      ...
      if (doc.exists) {
        final data = doc.data()!;
        debugPrint(
          '🔍 getSessionJoinStatus - Raw data: ${data['patientJoined']}, ${data['doctorJoined']}',
        );
  ...
  static Stream<Map<String, dynamic>> getSessionJoinStatusStream(
    String appointmentId,
  ) {
    return _firestore
        .collection('telemedicine_sessions')
  ...
  static Future<void> updatePatientLeftCall(String appointmentId) async {
    try {
      await FirebaseFirestore.instance
          .collection('telemedicine_sessions')
          .doc(appointmentId)
          .update({'patientJoined': false, 'lastPatientLeft': DateTime.now()});
